...
public:
	...
	void parse() {
		...
		if (get_token.cache_token == EXCLAMATION_MARK) {
			// If it's a unary expression
			root_node.expression_type = "Unary Expression";

			root_node.operator_symbol = "!";
			
			root_node.unitary_expr = new _ast_node_assignment_expression;
			...
			while (!ast_lexical.is_eof()) {
				...
				if (!(get_token.cache_token == UNKNOW_TOKEN ||
					(get_token.cache_token >= PLUS_SIGN_TOKEN &&
						get_token.cache_token <= MODULO_SIGN_TOKEN) ||
					get_token.cache_token != CONST_NUMBER ||
					get_token.cache_token != CONST_STRING
					)) {
					break;
				}
			...
			if (strcmp(sub_tree->ast_status.error_cursor->error_string, "$") != 0) {
				...
				root_node.unitary_expr->left_value_tree = new _ast_node_assignment_expression;

				if (sub_tree->root_node.right_value_tree != nullptr) {
					root_node.unitary_expr->left_value_tree->left_value_tree = sub_tree->root_node.left_value_tree;
					root_node.unitary_expr->left_value_tree->right_value_tree = sub_tree->root_node.right_value_tree;
				}
				else {
					root_node.unitary_expr->left_value_tree = sub_tree->root_node.left_value_tree;
				}

				root_node.unitary_expr->left_value_tree->operator_symbol = sub_tree->root_node.operator_symbol;
				root_node.unitary_expr->left_value_tree->expression_type = sub_tree->root_node.expression_type;

				root_node.unitary_expr->left_value_tree->function_callee = sub_tree->root_node.function_callee;
			}
			...
			if (sub_expression.empty() == false) {
				...
				if (strcmp(sub_tree->ast_status.error_cursor->error_string, "$") != 0) {
					...
					root_node.unitary_expr->right_value_tree = new _ast_node_assignment_expression;

					if (sub_tree->root_node.right_value_tree != nullptr) {
						root_node.unitary_expr->right_value_tree->left_value_tree = sub_tree->root_node.left_value_tree;
						root_node.unitary_expr->right_value_tree->right_value_tree = sub_tree->root_node.right_value_tree;
					}
					else {
						root_node.unitary_expr->right_value_tree = sub_tree->root_node.left_value_tree;
					}

					root_node.unitary_expr->right_value_tree->operator_symbol = sub_tree->root_node.operator_symbol;
					root_node.unitary_expr->right_value_tree->expression_type = sub_tree->root_node.expression_type;

					root_node.unitary_expr->right_value_tree->function_callee = sub_tree->root_node.function_callee;
				}
			}
		}
		else {
